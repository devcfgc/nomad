...
.PHONY: alex
alex: GOOS=$(shell go env GOOS)
alex: GOARCH=$(shell go env GOARCH)
alex: GOPATH=$(shell go env GOPATH)
alex: PROJECT_ROOT=C:\Users\vagrant\go\src\github.com\hashicorp\nomad
alex: DEV_TARGET=pkg\$(GOOS)_$(GOARCH)$(if $(HAS_LXC),-lxc)\$(BINARY)
alex: ## Build for the current development platform
	@cp foo $(PROJECT_ROOT)\bin\
